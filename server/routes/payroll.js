...
router.get('/:companySlug', authenticate, companyAccess, async (req, res) => {
  try {
    ...
    res.json(payrolls);
  } catch (err) { console.error('payroll.js error:', err); res.status(500).json({ error: err.message }); }
});
...
router.post('/:companySlug/calculate', authenticate, companyAccess, async (req, res) => {
  try {
    ...
    for (const emp of employees) {
      const existingPayslip = await db.prepare('SELECT id FROM payroll WHERE user_id = ? AND month = ? AND year = ? AND company_slug = ?').get(emp.user_id, month, year, req.params.companySlug);
      ...
      const advancesTotal = await db.prepare("SELECT COALESCE(SUM(amount),0) as total FROM salary_advances WHERE user_id = ? AND status = 'paid' AND company_slug = ? AND strftime('%m', created_at) = ? AND strftime('%Y', created_at) = ?")
        .get(emp.user_id, req.params.companySlug, String(month).padStart(2, '0'), String(year));
      ...
      await db.prepare('INSERT INTO payroll (id, user_id, month, year, base_salary, advances_deducted, late_penalties, net_salary, company_slug) VALUES (?,?,?,?,?,?,?,?,?)')
        .run(id, emp.user_id, month, year, emp.base_salary, advancesTotal ? advancesTotal.total || 0 : 0, latePenalty, Math.max(0, netSalary), req.params.companySlug);
