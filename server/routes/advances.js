...
router.post('/:companySlug', authenticate, (req, res) => {
  ...
  db.prepare('INSERT INTO salary_advances (id, user_id, amount, reason, repayment_terms) VALUES (?,?,?,?,?)')
  ...
  const adv = db.prepare('SELECT a.*, u.full_name as user_name FROM salary_advances a LEFT JOIN users u ON a.user_id = u.id WHERE a.id = ?').get(id);
  res.json(adv);
});
...
router.put('/:companySlug/:id/review', authenticate, (req, res) => {
  ...
  db.prepare("UPDATE salary_advances SET status = ?, reviewed_by = ?, reviewed_at = datetime('now') WHERE id = ?")
    .run(status, req.user.id, req.params.id);
  const adv = db.prepare('SELECT a.*, u.full_name as user_name FROM salary_advances a LEFT JOIN users u ON a.user_id = u.id WHERE a.id = ?').get(req.params.id);
  db.prepare('INSERT INTO notifications (id, user_id, title, message, type) VALUES (?,?,?,?,?)')
