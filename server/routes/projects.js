...
router.post('/:companySlug', authenticate, (req, res) => {
  ...
  db.prepare('INSERT INTO projects (id, title, description, client_name, order_value, down_payment, request_date, request_type_id, execution_method, created_by) VALUES (?,?,?,?,?,?,?,?,?,?)')
  ...
  const project = db.prepare('SELECT p.*, rt.name as request_type_name FROM projects p LEFT JOIN request_types rt ON p.request_type_id = rt.id WHERE p.id = ?').get(id);
  res.json(project);
});

router.put('/send-to-tasks/:companySlug/:id', authenticate, (req, res) => {
  const db = getCompanyDb(req.params.companySlug);
  const company = req.params.companySlug;
  const firstStage = FIRST_STAGES[company] || 'draft';
  db.prepare('UPDATE projects SET stage = ? WHERE id = ?').run(firstStage, req.params.id);
  const project = db.prepare('SELECT p.*, rt.name as request_type_name FROM projects p LEFT JOIN request_types rt ON p.request_type_id = rt.id WHERE p.id = ?').get(req.params.id);
  res.json(project);
});
...
router.delete('/:companySlug/:id', authenticate, (req, res) => {
  const db = getCompanyDb(req.params.companySlug);
