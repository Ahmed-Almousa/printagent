...
router.post('/:companySlug/clock-in', authenticate, async (req, res) => {
  ...
  await db.prepare('INSERT INTO attendance (id, user_id, date, clock_in, location_lat, location_lng) VALUES (?,?,?,?,?,?)')
...
router.post('/:companySlug/clock-out', authenticate, async (req, res) => {
  ...
  const now = new Date().toISOString();
  await db.prepare('UPDATE attendance SET clock_out = ? WHERE id = ?').run(now, record.id);
...
router.get('/:companySlug/today', authenticate, async (req, res) => {
  ...
  const employees = await db.prepare("SELECT id, full_name FROM employees WHERE is_active = 1").all();
